 ...
 */
export const DEFAULT_LLM_AGENT_TASKS: readonly LlmAgentTask[] = [
  {
    id: 'llm-review-needs-test',
    surface: 'reviewer',
    prompt:
      'A pull request adds this new function with no accompanying test:\n\n' +
...
async function readSurfaceGuidance(variantDir: string, surface: MutationSurface): Promise<string> {
  try {
 ...
 */
export async function runVariantTaskLlmAgent(
  ...
  const systemPromptAppend =
    `Additional ${task.surface} guidance for this session, from the repository's own policy file ` +
...
export async function runVariantTasksLlmAgent(
  variant: HarnessVariant,
  tasks: readonly LlmAgentTask[] = DEFAULT_LLM_AGENT_TASKS,
  timeoutMs = DEFAULT_LLM_TIMEOUT_MS,
): Promise<RunTrace[]> {
  const traces: RunTrace[] = [];
  for (const task of tasks) traces.push(await runVariantTaskLlmAgent(variant, task, timeoutMs));
  return traces;
}
