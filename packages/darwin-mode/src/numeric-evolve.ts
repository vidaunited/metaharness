...
import type {
  NumericArchiveRecord,
  NumericEvolutionConfig,
  NumericEvolutionResult,
  NumericVariant,
...
function genomeDistance(a: NumericVariant, baseline: NumericVariant): number {
  let sum = 0;
...
export async function evolveNumeric(config: NumericEvolutionConfig): Promise<NumericEvolutionResult> {
  ...
  let parents: NumericVariant[] = [baseline];

  for (let generation = 1; generation <= config.generations; generation++) {
    const children: NumericVariant[] = [];
        ...
        if (canCross && localIndex === 0) {
          ...
          children.push(
            makeVariant(
              ...
              genome,
              mutatedParams,
              mutatedParams.length > 0 ? `perturbed [${mutatedParams.join(', ')}]` : 'no-op perturbation (clamped to parent)',
            ),
          );
        }
      }
    ...
    for (let i = 0; i < children.length; i++) {
      ...
      if (!scores[i].regressed && improved) promoted.push(child);
    }
    await archive.save();
    ...
    if (promoted.length > 0) {
      ...
      const scoredRecords = archive.all().filter((r): r is NumericArchiveRecord & { score: NonNullable<NumericArchiveRecord['score']> } => r.score !== null && !r.score.regressed);
